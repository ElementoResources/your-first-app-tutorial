...
function StartPage(props) {
    ...
    return React.createElement(Page, {id: props.path},
        ...
        React.createElement(TextElement, {path: pathWith('Para2')}, 'This short guide will take you step-by-step through creating a really simple Elemento app, called Hello You.  The app just asks for your name,  and then displays "Hello, Jo" (or whatever you tell it you are called).'),
        React.createElement(TextElement, {path: pathWith('Para3')}, `You will need:
- A computer with a good size screen - Full HD (1920 x 1024) is best
- To be using an up to date version of the Chrome browser
- About 5 minutes`),
...
function HowtoUsePage(props) {
    ...
    const app = Elemento.useGetObjectState('app')
    const {ShowPage} = app
    const Previous_action = React.useCallback(() => {
        ShowPage(StartPage)
    }, [])
    const NextButton_action = React.useCallback(() => {
        ShowPage(WhatYouSeePage)
    }, [])

    return React.createElement(Page, {id: props.path},
        React.createElement(TextElement, {path: pathWith('Title'), fontSize: 24}, 'How to Use this Tutorial'),
        React.createElement(TextElement, {path: pathWith('Para1')}, 'There is a lot to see on this screen - the next page explains what you are looking at.'),
...
function WhatYouSeePage(props) {
    ...
    const app = Elemento.useGetObjectState('app')
    const {ShowPage} = app
    const ShowMe1_action = React.useCallback(() => {
        Editor.Show('treeItem')
    }, [])
    const ShowMe2_action = React.useCallback(() => {
        Editor.Click('treeItem', 'Main Page')
        Editor.Show('propertyField')
    }, [])
    const ShowMe3_action = React.useCallback(() => {
        Preview.Show('MainApp')
    }, [])
    const ShowMe4_action = React.useCallback(() => {
        Editor.Show('button')
    }, [])
    const Previous_action = React.useCallback(() => {
        ShowPage(HowtoUsePage)
...
function Step1Page(props) {
    ...
    const app = Elemento.useGetObjectState('app')
    const {ShowPage} = app
    const ShowItem_action = React.useCallback(() => {
        Editor.Show('treeItem', 'Main Page')
    }, [])
    const ExpandItem_action = React.useCallback(() => {
        Editor.EnsureTreeItemsExpanded('treeItem', 'Main Page')
    }, [])
    const Showlink_action = React.useCallback(() => {
        Editor.Click('treeItem', 'Title')
    }, [])
    const ShowMe4_action = React.useCallback(() => {
        Editor.Click('propertyField', 'Content')
    }, [])
    const ShowMe5_action = React.useCallback(() => {
        Editor.SetValue('propertyField', 'Content', 'Hello You')
    }, [])
    const ShowMe6_action = React.useCallback(() => {
        Preview.Show('Title')
    }, [])
    const Previous_action = React.useCallback(() => {
        ShowPage(WhatYouSeePage)
...
function Step2Page(props) {
    ...
    const app = Elemento.useGetObjectState('app')
    const {ShowPage} = app
    const ShowMe21_action = React.useCallback(() => {
        Editor.Show('treeItem', 'Title')
    }, [])
    const ShowMe22_action = React.useCallback(() => {
        Editor.Click('button', 'Insert')
    }, [])
    const ShowMe23_action = React.useCallback(() => {
        Editor.Click('menuItem', 'After')
    }, [])
    const ShowMe24_action = React.useCallback(() => {
        Editor.Click('menuItem', 'Text Input')
    }, [])
    const ShowMe26_action = React.useCallback(() => {
        Editor.Click('propertyField', 'Name')
    }, [])
    const ShowMe25_action = React.useCallback(() => {
        Editor.SetValue('propertyField', 'Name', 'Your Name')
    }, [])
    const Previous2_action = React.useCallback(() => {
        ShowPage(Step1Page)
    }, [])
    const Next2_action = React.useCallback(() => {
        ShowPage(Step3Page)
    }, [])
    ...
    return React.createElement(Page, {id: props.path},
        ...
        React.createElement(Layout, {path: pathWith('Step6'), horizontal: true, wrap: false},
            React.createElement(TextElement, {path: pathWith('Instruction')}, `Change the Name to 'Your Name'.
...
function Step3Page(props) {
    ...
    const app = Elemento.useGetObjectState('app')
    const {ShowPage} = app
    const ShowMe31_action = React.useCallback(() => {
        Editor.Show('treeItem', 'Your Name')
    }, [])
    const ShowMe32_action = React.useCallback(() => {
        Editor.Click('button', 'Insert')
    }, [])
    const ShowMe33_action = React.useCallback(() => {
        Editor.Click('menuItem', 'After')
    }, [])
    const ShowMe34_action = React.useCallback(() => {
        Editor.Click('menuItem', 'Text$')
    }, [])
    const ShowMe35_action = React.useCallback(() => {
        Editor.Click('propertyTypeButton', 'Content')
    }, [])
    const ShowMe36_action = React.useCallback(() => {
        Editor.SetValue('propertyField', 'Content', "'Hello there, ' + YourName")
    }, [])
    const ShowMe37_action = React.useCallback(() => {
        Preview.SetValue('YourName', 'Alexandra')
    }, [])
    const Previous3_action = React.useCallback(() => {
        ShowPage(Step2Page)
    }, [])
    const Next3_action = React.useCallback(() => {
        ShowPage(FinishPage)
    }, [])
...
`),
        ...
        React.createElement(TextElement, {path: pathWith('Step7')}, 'Your text here'),
        React.createElement(Layout, {path: pathWith('Buttons'), horizontal: true, wrap: false},
...
function FinishPage(props) {
    ...
    const BacktoStartButton_action = React.useCallback(() => {
        ShowPage(StartPage)
    }, [])
...
export default function Tutorial(props) {
    ...
    const {Editor, Preview} = Elemento
    const pages = {StartPage, HowtoUsePage, WhatYouSeePage, Step1Page, Step2Page, Step3Page, FinishPage}
    const {appContext} = props
    const app = Elemento.useObjectState('app', new App.State({pages, appContext}))
    const Tutorial_startupAction = React.useCallback(() => {
        Editor.SetOptions({showBeforeActions: true,
